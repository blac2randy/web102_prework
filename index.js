...
contributionsCard.innerHTML = `<span>${totalContributions.toLocaleString()}</span> Contributions`;
...
raisedCard.innerHTML = `<span>$${totalRaised.toLocaleString()}</span> Raised`;
...
gamesCard.innerHTML = `<span>${totalGames.toLocaleString()}</span> Games`;
/*************************************************************************************
...
const unfundedGamesArray = [...Array(companyInfo.totalGames)].filter(
    (_, index) => index >= companyInfo.fundedGames
  );
  

// create a string that explains the number of unfunded games using the ternary operator
const unfundedGamesInfo = `We have ${
    unfundedGamesCount > 0
      ? `${unfundedGamesCount} unfunded game${unfundedGamesCount > 1 ? "s" : ""}`
      : "no unfunded games"
  } out of ${companyInfo.totalGames} total games.`;

// create a new DOM element containing the template string and append it to the description container

const additionalInfoElement = document.createElement("p");
additionalInfoElement.textContent = `Welcome to ${companyInfo.companyName}! ${unfundedGamesInfo}`;
...
firstGameElement.innerHTML = `<h3>${firstGame.name}</h3><p>Pledged: ${firstGame.pledged}</p>`;
firstGameContainer.appendChild(firstGameElement);


// do the same for the runner up item
const secondGameElement = document.createElement("div");
secondGameElement.innerHTML = `<h3>${secondGame.name}</h3> <p>Pledged: ${secondGame.pledged}</p>`;
secondGameContainer.appendChild(secondGameElement);

// Unsure why the Top unfunded game and runner up is not displaying at times.